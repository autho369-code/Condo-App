...
- **Role identity result:** eight staging identities now sign in through Supabase Auth and resolve through the real `me()` RPC: Company Admin A/B, Manager A, Board A, Owner A/B, and Vendor A/B. Each resolved its expected role linkage and portfolio. Each identity was also denied a direct RLS read of the other marked portfolio. This verifies authentication, primary role resolution, and one top-level isolation boundary; it does not yet certify every page/action permission.
