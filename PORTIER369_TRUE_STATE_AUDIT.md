...
- **Monthly financial package:** the manager report workspace now offers one authenticated association-scoped PDF containing a cover page plus trial balance, balance sheet, income statement, budget vs actual, A/R aging, delinquency summary, A/P aging, and bank reconciliation sections. Each section is generated from the supported live-export processor under the signed-in user's RLS scope; empty sections are labeled rather than filled with invented data. A deterministic sample produced a readable nine-page landscape Letter PDF with section headings, reporting period, table headers, and page numbers. The full regression gate passes 133/133 tests, the production build, TypeScript/lint checks, secret scan, and diff validation. Protected-preview/staging download evidence is still required before this item is considered deployed-path verified.
