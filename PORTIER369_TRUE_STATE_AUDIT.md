...
- **Check PDF and preview evidence:** a manager signed into the protected Vercel branch preview, selected one of two approved fixture bills, acknowledged authorization, issued check #5001, and reached the immutable one-check preview containing the expected payee, amount, amount-in-words, date, invoice, GL, signer label, and two voucher stubs. The deployed authenticated `Download PDF` route returned `check-run-5001.pdf`; metadata, text extraction, and a 144-DPI render confirmed one 612x792-point Letter page, three aligned panels, all expected fields, legible text, no clipping/overlap, and no broken glyphs. Two new PDF tests pass. The inherited fake MICR-like glyph line was removed; both preview and PDF now state that approved preprinted MICR stock is required. The deterministic seed then removed the issued test check and restored staging.
- **Financial PDF labeling repair:** queued PDF exports now receive the actual report-definition name, portfolio or association scope, reporting period/as-of date, humanized column labels, and repeated page headers/page numbers instead of the inherited generic `Portier369 report` heading. This code path covers the verified balance sheet, income statement, trial balance, general ledger, A/R aging, delinquency, A/P aging, budget, and bank-reconciliation processors.
- **Monthly financial package:** the manager report workspace now offers one authenticated association-scoped PDF containing a cover page plus trial balance, balance sheet, income statement, budget vs actual, A/R aging, delinquency summary, A/P aging, and bank reconciliation sections. Each section is generated from the supported live-export processor under the signed-in user's RLS scope; empty sections are labeled rather than filled with invented data. The first protected-preview download exposed a wide budget continuation and header-placement defect; the package now selects board-facing columns, calculates selected-month and YTD budget variances, omits internal record IDs, and adds headers after table pagination using the actual page width. A manager-authenticated second download from the exact `c4886ff` protected preview produced a 65,263-byte, nine-page landscape Letter PDF. Raster inspection and text/coordinate extraction verified the cover, all eight section titles, association, period, correct page numbers, readable tables, June/YTD budget variances, and absence of internal charge, bill, and reconciliation IDs. The full regression gate passes 134/134 tests, the production build, TypeScript/lint checks, secret scan, and diff validation.
