...
- **Monthly financial package:** the manager report workspace now offers one authenticated association-scoped PDF containing a cover page plus trial balance, balance sheet, income statement, budget vs actual, A/R aging, delinquency summary, A/P aging, and bank reconciliation sections. Each section is generated from the supported live-export processor under the signed-in user's RLS scope; empty sections are labeled rather than filled with invented data. The first protected-preview download exposed a wide budget continuation and alternating-page header-placement defect; the package now selects board-facing columns, calculates selected-month and YTD budget variances, omits internal record IDs, and adds headers after table pagination using the actual page width. The full regression gate passes 134/134 tests, the production build, TypeScript/lint checks, secret scan, and diff validation. A second protected-preview download is required before the layout repair is considered deployed-path verified.
