...
| --- | --- | --- | --- |
| Release | VERIFIED WORKING | 166 tests / 50 files, TypeScript, lint without errors, route/dashboard audits, migration validation, secret scan, build, GitHub CI, and Vercel pass. | Keep checks required on the approved release commit. |
...
| Authentication/routing | VERIFIED for six personas | Operator, Company Admin, Manager, Board, Owner, and Vendor authenticated in isolated browser sessions; live stale-token revocation removed every capability/protected portfolio read and all profiles recovered. | Test invitation/reset abuse and any explicitly supported tenant persona. |
...
| Documents | PARTIALLY VERIFIED | Migration-managed private bucket; signed PDF upload/download passes; public read, unsigned upload, executable MIME, and cleanup checks pass. | Complete per-workflow cross-entity signed-capability and shared/expired URL tests. |
