...
- ⏳ **Platform remittance details (operator billing)** — decide how management companies pay Portier369 (ACH details / mailing address / Bill.com or similar link). Today operators can generate + email + mark-paid invoices, but the invoice email and the company Billing page say "contact us for remittance details" because nothing is stored. Once decided, bake it into the invoice email body (`sendInvoice` in `app/platform-operator/companies/actions.ts`) and the company-admin Billing page.
