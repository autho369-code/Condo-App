...
> - **Teachable workflow automation (Flows): MISSING → HAVE.** /automation-center/flows — 5 triggers (charge overdue, WO stale, violation stale, insurance expiring, ARC pending) × 4 actions (white-labeled owner email w/ template variables, manager alert, apply late fee via assess_late_fee, raise WO priority). Hourly cron; fires at most once per subject ever (DB unique constraint); full run ledger. This was AppFolio's last clear AI/automation edge (Realm-X Flows, gated behind Max) — ours ships on every tier.
