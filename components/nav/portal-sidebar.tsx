  ...
  return (
    <aside className="flex h-screen w-52 flex-shrink-0 flex-col border-r border-slate-800 bg-[#0B1121] overflow-hidden">
      <div className="border-b border-slate-800 px-4 py-3 flex-shrink-0">
        ...
        <div className="text-xs text-slate-500 mt-0.5">Owner Portal</div>
      </div>

      <nav className="flex-1 overflow-y-auto py-1">
        {portalModules.map((m) => (
          <Link key={m.href} href={m.href}
            className={"flex items-center px-4 py-2 text-sm " + (active(m.href) ? 'bg-emerald-500/10 text-emerald-400 font-medium' : 'text-slate-400 hover:bg-slate-800/50 hover:text-slate-200')}>
            {m.label}
          </Link>
        ))}
      </nav>
