...
-- Align subscription tiers with the public pricing offer:
-- core->foundation, plus->growth, max->portfolio, plus a new 'enterprise' tier.
-- RENAME VALUE keeps each value's OID, so existing rows + column defaults remap
-- automatically and no dependent objects break.
...
-- provision_portfolio's p_tier default referenced the old 'core' value.
