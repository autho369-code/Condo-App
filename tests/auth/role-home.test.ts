...
describe('roleHome precedence', () => {
  it('routes each role to its own surface', () => {
    ...
    expect(roleHome(me({ is_tenant: true, tenant_id: 't1' }))).toBe('/resident');
  });
  ...
  it('higher-privilege roles win when a user has several', () => {
    ...
    // An owner/occupant identity retains the owner financial portal as home.
    expect(roleHome(me({ owner_id: 'o1', is_tenant: true, tenant_id: 't1' }))).toBe('/portal');
  });
