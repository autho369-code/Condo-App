      ...
      <div className="overflow-x-auto rounded-2xl border border-gray-200/70 bg-white shadow-[0_1px_2px_rgba(16,24,40,0.04)]">
        <table className="w-full text-sm">
          <thead className="border-b border-gray-100 bg-gray-50/60 text-[11px] uppercase tracking-wide text-gray-500">
            <tr>
              <th className="px-4 py-2.5 text-left font-medium">Work Order</th>
              ...
              all.map((w: any) => (
                <tr key={w.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60">
                  ...
                  <td className="px-4 py-3 text-center">
                    <StatusChip tone={priorityTone(w.priority)}>{w.priority ?? '—'}</StatusChip>
                  </td>
                  <td className="px-4 py-3 text-center"><Badge status={w.status} /></td>
                  <td className="px-4 py-3 text-[13px] text-gray-700">{w.vendors?.name ?? '—'}</td>
