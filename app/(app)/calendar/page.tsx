...
const statusTone: Record<string, string> = {
  scheduled: 'bg-blue-100 text-champagne-700',
  notice_sent: 'bg-cyan-100 text-cyan-700',
  reminder_sent: 'bg-indigo-100 text-indigo-700',
  completed: 'bg-green-100 text-green-700',
  canceled: 'bg-cream-100 text-ink-600',
  failed_notification: 'bg-red-100 text-bordeaux-700',
};
...
export default async function CalendarPage({
  ...
  return (
    <div className="flex h-full">
      <main className="flex-1 overflow-y-auto bg-cream-50 px-8 py-6">
        <div className="mb-6 flex items-start justify-between gap-6">
          <div>
            <div className="text-xs font-semibold uppercase tracking-wider text-ink-500">Operations Calendar</div>
            <h1 className="mt-1 text-2xl font-semibold text-ink-900">Association calendar</h1>
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <Metric label="Critical / deadline events" value={critical} tone="text-champagne-700" />
          <Metric label="Draft notices" value={rows.filter((event: any) => event.operations_status === 'scheduled' && event.public_notice_text).length} />
          <Metric label="Needs communication" value={rows.filter((event: any) => !event.public_notice_text).length} tone="text-bordeaux-700" />
        </div>

        <div className="mb-5 flex flex-wrap items-center gap-3 rounded-lg border border-ink-100 bg-white p-4">
          <form action="/calendar" className="flex flex-wrap items-center gap-3">
            ...
            <select name="type" defaultValue={sp.type ?? ''} className="h-9 rounded border border-ink-200 bg-white px-3 text-sm">
            ...
            {(sp.assoc || sp.type) && (
              <Link href="/calendar" className="text-sm text-ink-500 hover:text-ink-900">Clear</Link>
            )}
          </form>
        </div>
        ...
        {rows.length ? (
            ...
            <tbody>
              {rows.map((event: any) => (
                <TR key={event.id}>
                  ...
                  <TD>
                    <div className="font-medium text-ink-900">{event.title}</div>
                    <div className="text-xs text-ink-500">{eventTypeLabel(event.event_type)}</div>
                  </TD>
                  <TD className="text-sm text-ink-700">{event.associations?.name ?? 'Portfolio-wide'}</TD>
                  <TD className="text-sm text-ink-600">{event.location ?? '-'}</TD>
                  <TD>
                    <span className={`rounded px-2 py-0.5 text-xs capitalize ${statusTone[event.operations_status ?? 'scheduled'] ?? 'bg-cream-100 text-ink-600'}`}>
                      {(event.operations_status ?? 'scheduled').replaceAll('_', ' ')}
                    </span>
                  </TD>
                  <TD className="max-w-md text-sm text-ink-600">
                    {event.public_notice_text ? <span className="line-clamp-2">{event.public_notice_text}</span> : <span className="text-bordeaux-600">Needs draft</span>}
                  </TD>
                </TR>
              ))}
            </tbody>
          ...
          <div className="rounded-lg border border-dashed border-ink-200 bg-white px-6 py-12 text-center">
            <h2 className="text-base font-semibold text-ink-900">No upcoming events found</h2>
            <p className="mt-1 text-sm text-ink-500">Create the first event to generate notices, reminders, and follow-up tasks.</p>
          </div>
        )}
      </main>

      <aside className="w-80 shrink-0 overflow-y-auto border-l border-ink-100 bg-white px-6 py-6">
        <h2 className="text-sm font-semibold text-ink-900">AIOS workflow examples</h2>
        ...
        <div className="mt-6 rounded-lg border border-blue-100 bg-champagne-50 p-4 text-sm text-blue-900">
