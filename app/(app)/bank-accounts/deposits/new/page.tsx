    ...
    >
      <form className="rounded border border-gray-200 bg-white p-5">
        <div className="grid gap-4 md:grid-cols-2">
          <label className="text-sm font-medium text-gray-700">Bank account<select className="mt-1 h-10 w-full rounded border border-gray-300 bg-white px-3 text-sm"><option>Select account</option>{(accounts ?? []).map((row: any) => <option key={row.id}>{row.name}</option>)}</select></label>
          <label className="text-sm font-medium text-gray-700">Association<select className="mt-1 h-10 w-full rounded border border-gray-300 bg-white px-3 text-sm"><option>All associations</option>{(associations ?? []).map((row: any) => <option key={row.id}>{row.name}</option>)}</select></label>
        </div>
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          <input className="h-10 rounded border border-gray-300 px-3 text-sm" placeholder="Unit or owner" />
