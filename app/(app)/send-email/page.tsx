    ...
    <div className="mx-auto max-w-3xl px-8 py-6">
      <div className="rounded-lg border border-ink-100 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-ink-100 px-6 py-4">
          <h1 className="text-xl font-semibold text-ink-900">Send Email</h1>
          <Link href={sp.return_to && sp.return_to.startsWith('/') ? sp.return_to : '/associations'}
            className="text-ink-400 hover:text-ink-600" aria-label="Close">×</Link>
        </div>
        ...
        <form action={sendEmail as any} className="space-y-5 px-6 py-5">
          ...
          {/* From */}
          <div>
            <label className="mb-1 block text-sm font-medium text-ink-700">From <span className="text-red-500">*</span></label>
            <input
              value={fromEmail}
              readOnly
              className="h-10 w-full rounded border border-ink-200 bg-cream-100 px-3 text-sm text-ink-700"
            />
            <label className="mt-2 flex items-center gap-2 text-sm text-ink-700">
              <input type="checkbox" name="from_donotreply" />
              Send from donotreply@portier369.com
            </label>
          </div>

          {/* Association */}
          <div>
            <label htmlFor="association_id" className="mb-1 block text-sm font-medium text-ink-700">
          ...
          {/* Cc */}
          <div>
            <label htmlFor="cc" className="mb-1 block text-sm font-medium text-ink-700">Cc</label>
            <input
              id="cc"
              name="cc"
          ...
          <div>
            <label htmlFor="additional_recipients" className="mb-1 block text-sm font-medium text-ink-700">
              Additional Recipients
            </label>
            <input
          ...
          {/* Subject */}
          <div>
            <label htmlFor="subject" className="mb-1 block text-sm font-medium text-ink-700">
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-ink-700">Attachments</label>
            <div className="rounded border-2 border-dashed border-ink-200 bg-cream-50 px-4 py-6 text-center text-sm text-ink-600">
              <strong>Drag Files Here</strong>
              <span className="mx-3 text-ink-400">or</span>
              <label className="inline-block cursor-pointer rounded border border-ink-200 bg-white px-3 py-1.5 font-medium text-ink-700 hover:bg-cream-50">
                Choose Files to Add
          ...
          <div className="flex items-center justify-between border-t border-ink-100 pt-4">
            <div className="flex gap-3">
              ...
              <Link
                ...
                className="inline-flex items-center justify-center rounded-md border border-ink-200 bg-white px-5 py-2 text-sm font-medium text-ink-700 hover:bg-cream-50"
              >
                Cancel
              </Link>
            </div>
            <Link href="/settings" className="text-sm text-champagne-700 hover:underline">
...
function RecipientOption({
  ...
  return (
    <label className="flex cursor-pointer items-start gap-2 rounded border border-ink-100 bg-white px-3 py-2 hover:border-brand-500 has-[:checked]:border-brand-500 has-[:checked]:bg-brand-50">
      ...
      <div className="flex-1 text-sm">
        ...
        <div className="text-xs text-ink-500">{count} recipient{count === 1 ? '' : 's'} with email on file</div>
      </div>
    </label>
  );
}
