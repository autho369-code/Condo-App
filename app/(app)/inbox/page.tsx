          ...
          <tbody>
            {rows.map((m: any) => (
              <TR key={m.id}>
                ...
                <TD><span className={`rounded px-2 py-0.5 text-xs ${m.direction === 'inbound' ? 'bg-blue-100 text-champagne-700' : 'bg-cream-100 text-ink-700'}`}>{m.direction}</span></TD>
                ...
                <TD className="text-xs capitalize text-ink-500">{m.status}</TD>
              </TR>
            ))}
          </tbody>
        </Table>
      ) : (
        <p className="rounded border border-ink-100 bg-white px-6 py-8 text-center text-sm text-ink-500">No SMS activity.</p>
      )}
    </ModulePage>
  );
}
