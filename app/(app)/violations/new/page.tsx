...
export default async function NewViolationPage() {
  ...
  return (
    <DataWorkspace
      ...
      actions={<Link href="/violations" className="text-sm font-medium text-slate-400 hover:text-gray-950">Cancel</Link>}
      rail={<NoticePreview />}
    >
      <form className="space-y-5">
        ...
        <Section title="Fine and evidence">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="rounded border border-dashed border-gray-300 bg-gray-50 px-4 py-6 text-center text-sm text-slate-400">Evidence and attachments will be added after the draft is saved.</div>
          </div>
        </Section>
...
function NoticePreview() {
  return <div className="space-y-3 text-sm text-slate-400"><h2 className="text-sm font-semibold text-gray-950">Draft notice preview</h2><p>No email, letter, or owner communication is sent from this screen. Preview first, then confirm outbound delivery from the saved record.</p></div>;
}
