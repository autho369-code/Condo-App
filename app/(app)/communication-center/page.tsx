...
const statusTone: Record<string, string> = {
  draft: 'bg-amber-100 text-champagne-700',
  queued: 'bg-blue-100 text-champagne-700',
  ...
  return (
    <div className="h-full overflow-y-auto bg-cream-50 px-8 py-6">
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 md:grid-cols-4">
        ...
        <Metric label="Drafts awaiting approval" value={drafts} tone="text-champagne-700" />
        <Metric label="Failed delivery" value={failed} tone="text-bordeaux-700" />
      ...
      {rows.length ? (
          ...
          <tbody>
            {rows.map((message: any) => (
              <TR key={message.id}>
                ...
                <TD className="uppercase text-xs font-semibold text-ink-600">{message.channel}</TD>
                <TD>
                  <div className="text-sm capitalize text-ink-900">{String(message.recipient_group ?? '').replaceAll('_', ' ')}</div>
                  <div className="text-xs text-ink-500">{message.recipient_email ?? message.recipient_phone ?? 'Resolved at send time'}</div>
                </TD>
                <TD className="text-sm text-ink-700">{message.associations?.name ?? 'Portfolio-wide'}</TD>
                <TD className="max-w-xl">
                  <div className="font-medium text-ink-900">{message.subject ?? 'No subject'}</div>
                  <div className="mt-1 line-clamp-2 text-sm text-ink-500">{message.body}</div>
                </TD>
                <TD>
                  <span className={`rounded px-2 py-0.5 text-xs capitalize ${statusTone[message.status] ?? 'bg-cream-100 text-ink-600'}`}>
                    {message.status}
                  </span>
                </TD>
              </TR>
        ...
        <div className="rounded-lg border border-dashed border-ink-200 bg-white px-6 py-12 text-center">
          <h2 className="text-base font-semibold text-ink-900">No communications yet</h2>
          <p className="mt-1 text-sm text-ink-500">Calendar events and manual emails will create draft messages here.</p>
        </div>
      )}
    </div>
  );
