...
export default async function VendorAchPage({
  ...
  return (
    <DataWorkspace
      ...
      rail={<div className="rounded border border-amber-200 bg-champagne-50 p-3 text-xs text-amber-800">Vendor ACH setup is confirmation-gated because it changes bank payout behavior.</div>}
    >
            ...
            {rows.map((vendor: any) => {
              ...
              return (
                <TR key={vendor.id} className="hover:bg-cream-50">
                  ...
                  <TD><StatusChip tone={hasBank ? 'success' : 'warning'}>{hasBank ? 'Bank info on file' : 'Missing bank info'}</StatusChip><div className="mt-1 text-xs text-ink-500">{vendor.savings_account ? 'Savings' : 'Checking'} account</div></TD>
                  <TD><StatusChip tone={vendor.is_auto_pay ? 'info' : 'neutral'}>{vendor.is_auto_pay ? 'Enabled' : 'Not enabled'}</StatusChip><div className="mt-1 text-xs text-ink-500">Setup {date(vendor.auto_pay_setup_at)}</div></TD>
                  <TD><Link href={`/vendors/forms?vendor=${vendor.id}&template=vendor_bank_account`} className="text-sm font-medium text-champagne-700 hover:underline">Prepare bank form</Link></TD>
                </TR>
              );
            })}
          </tbody>
