  ...
  return (
    <div className="mx-auto h-full max-w-3xl overflow-y-auto px-8 py-6">
      <div className="mb-4 text-xs font-semibold uppercase tracking-wider text-gray-500">
        <Link href="/sms" className="hover:text-brand-600">SMS</Link> / <Link href="/sms/templates" className="hover:text-brand-600">Templates</Link> / {template.name}
      </div>
      <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
        ...
        <form action={saveTemplate as any} className="space-y-5 px-6 py-5">
          ...
          <div>
            ...
            <input id="name" name="name" required defaultValue={template.name} className="h-10 w-full rounded border border-gray-300 bg-white px-3 text-sm" />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="channel" className="mb-1 block text-sm font-medium text-gray-700">Channel</label>
              <select id="channel" name="channel" defaultValue={template.channel} className="h-10 w-full rounded border border-gray-300 bg-white px-3 text-sm">
            ...
            <div>
              ...
              <select id="category" name="category" defaultValue={template.category ?? 'general'} className="h-10 w-full rounded border border-gray-300 bg-white px-3 text-sm">
          ...
          <div>
            ...
            <input id="subject" name="subject" defaultValue={template.subject ?? ''} className="h-10 w-full rounded border border-gray-300 bg-white px-3 text-sm" placeholder="Email subject line" />
          </div>
          ...
          <div>
            ...
            <textarea id="body" name="body" required rows={8} defaultValue={template.body} className="w-full rounded border border-gray-300 bg-white px-3 py-2 text-sm font-mono" />
