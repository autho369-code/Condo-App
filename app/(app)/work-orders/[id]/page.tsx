...
function statusBadge(s: string) {
  ...
  if (s === 'cancelled') return 'bg-gray-100 text-slate-400 line-through';
  ...
  return (
    <Workspace
        ...
        <WorkspaceHeader
          ...
          subtitle={
            <>
              ...
              {wo.category && <span className="mr-1 rounded bg-gray-100 px-2 py-0.5 text-xs capitalize text-slate-400">{wo.category.replace(/_/g, ' ')}</span>}
              {wo.trade && <span className="mr-1 rounded bg-gray-100 px-2 py-0.5 text-xs capitalize text-slate-400">{wo.trade.replace(/_/g, ' ')}</span>}
            </>
          }
        />
      }
      rail={
        <>
          {/* Status transitions */}
          <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-400">Status</div>
                ...
                <form key={s} action={updateWorkOrderStatus.bind(null, id, s, undefined) as any}>
                  <button type="submit"
                    className={`w-full rounded-md border px-3 py-1.5 text-left text-sm transition ${
                      isDestr ? 'border-gray-200 text-slate-400 hover:border-red-300 hover:bg-red-50'
          ...
          {/* Vendor */}
          <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-400">{vendor ? 'Vendor' : 'Assign vendor'}</div>
          {vendor ? (
            <div className="rounded-md border border-gray-200 p-3 text-sm">
              <div className="font-medium">{vendor.name}</div>
              <div className="text-xs uppercase text-slate-400">{vendor.trade}</div>
    ...
    >
      <Section title="Work details">
        <dl className="grid grid-cols-2 gap-x-4 gap-y-3 px-5 py-4 text-sm">
          <div><dt className="text-xs uppercase tracking-wider text-slate-400">Priority</dt><dd className="mt-0.5 font-medium capitalize">{wo.priority}</dd></div>
          <div><dt className="text-xs uppercase tracking-wider text-slate-400">Category</dt><dd className="mt-0.5 capitalize">{wo.category?.replace(/_/g, ' ') ?? 'â€”'}</dd></div>
          <div><dt className="text-xs uppercase tracking-wider text-slate-400">Trade</dt><dd className="mt-0.5 capitalize">{wo.trade?.replace(/_/g, ' ') ?? 'â€”'}</dd></div>
          <div><dt className="text-xs uppercase tracking-wider text-slate-400">Scheduled</dt><dd className="mt-0.5">{date(wo.scheduled_date)} {wo.scheduled_time ?? ''}</dd></div>
          <div><dt className="text-xs uppercase tracking-wider text-slate-400">Assigned to</dt><dd className="mt-0.5">{wo.assigned_to ?? 'â€”'}</dd></div>
          <div><dt className="text-xs uppercase tracking-wider text-slate-400">Requested by</dt><dd className="mt-0.5">{wo.requested_by ?? 'â€”'}</dd></div>
          <div className="col-span-2"><dt className="text-xs uppercase tracking-wider text-slate-400">Issue</dt><dd className="mt-0.5 whitespace-pre-wrap">{wo.issue ?? wo.description ?? 'â€”'}</dd></div>
          {wo.vendor_instructions && <div className="col-span-2"><dt className="text-xs uppercase tracking-wider text-slate-400">Vendor instructions</dt><dd className="mt-0.5 whitespace-pre-wrap">{wo.vendor_instructions}</dd></div>}
          {wo.owner_availability && <div className="col-span-2"><dt className="text-xs uppercase tracking-wider text-slate-400">Access / availability</dt><dd className="mt-0.5">{wo.owner_availability}</dd></div>}
          {wo.internal_notes && <div className="col-span-2"><dt className="text-xs uppercase tracking-wider text-slate-400">Internal notes</dt><dd className="mt-0.5 whitespace-pre-wrap">{wo.internal_notes}</dd></div>}
        </dl>
        ...
        <Section title={`Parent service request #${sr.number ?? sr.id.slice(0, 8)}`}>
          <div className="px-5 py-4">
            ...
            <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
              <div><dt className="text-slate-400">Priority</dt><dd className="capitalize">{sr.priority}</dd></div>
              <div><dt className="text-slate-400">Source</dt><dd className="capitalize">{sr.source}</dd></div>
              <div><dt className="text-slate-400">Status</dt><dd className="capitalize">{sr.status}</dd></div>
              {(sr.owners as any)?.full_name && <div><dt className="text-slate-400">Submitted by</dt><dd>{(sr.owners as any).full_name} ({(sr.owners as any).email})</dd></div>}
            </dl>
          </div>
        </Section>
      )}
      ...
      <Section
        title="Labor entries"
        actions={laborTotal > 0 ? <div className="text-xs text-slate-400">Total <span className="font-semibold text-gray-900">{money(laborTotal)}</span></div> : null}
      >
            ...
            <tbody>
              {labor.map((l: any) => (
                <TR key={l.id}>
                  ...
                  <TD className="text-slate-400">{l.description}</TD>
                </TR>
              ))}
            </tbody>
          </Table>
        ) : <p className="px-5 py-4 text-sm text-slate-400">No labor entries yet.</p>}
      ...
      <Section title="Estimates">
        ...
        ) : <p className="px-5 py-4 text-sm text-slate-400">No estimates yet.</p>}
        ...
        <ul className="space-y-3 border-l-2 border-gray-100 px-5 py-4 pl-8">
          {(updates ?? []).map((u: any) => (
            <li key={u.id} className="relative">
              ...
              {u.new_status && <div className="text-xs text-slate-400">â†’ status: <span className="capitalize">{u.new_status.replace(/_/g, ' ')}</span></div>}
              <div className="text-xs text-gray-400">{date(u.created_at)}</div>
            </li>
          ))}
          {!updates?.length && <li className="text-sm text-slate-400">No activity yet.</li>}
        </ul>
