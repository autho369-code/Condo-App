  ...
  return (
    <div className="h-full overflow-y-auto bg-gray-50 px-8 py-6">
      <div className="mb-6 flex items-start justify-between gap-6">
        <div>
          <div className="text-xs font-semibold uppercase tracking-wider text-slate-400">Automation Center</div>
          <h1 className="mt-1 text-2xl font-semibold text-gray-900">Reminders, drafts, and follow-up work</h1>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <section>
          <h2 className="mb-3 text-sm font-semibold text-gray-900">Reminder queue</h2>
          {reminderRows.length ? (
            <Table>
              <THead>
                <TR>
                ...
                {reminderRows.map((reminder: any) => (
                  <TR key={reminder.id}>
                    ...
                    <TD>
                      ...
                      <div className="text-xs text-slate-400">{reminder.associations?.name ?? 'Portfolio-wide'}</div>
                    </TD>
                    <TD className="text-sm capitalize">{String(reminder.recipient_group).replaceAll('_', ' ')}</TD>
                    <TD className="max-w-xs text-sm text-slate-400">{String(reminder.action).replaceAll(',', ', ')}</TD>
                    <TD><span className="rounded bg-blue-100 px-2 py-0.5 text-xs capitalize text-blue-700">{reminder.status}</span></TD>
                  </TR>
                ))}
              </tbody>
            </Table>
        ...
        <section>
          <h2 className="mb-3 text-sm font-semibold text-gray-900">Follow-up tasks</h2>
          {taskRows.length ? (
            <Table>
              <THead>
                <TR>
                ...
                {taskRows.map((task: any) => (
                  <TR key={task.id}>
                    ...
                    <TD>
                      ...
                      <div className="line-clamp-2 text-xs text-slate-400">{task.description}</div>
                    </TD>
                    <TD className="text-sm text-gray-700">{task.associations?.name ?? 'Portfolio-wide'}</TD>
                    <TD><span className="rounded bg-amber-100 px-2 py-0.5 text-xs capitalize text-amber-700">{task.status}</span></TD>
                  </TR>
                ))}
              </tbody>
            </Table>
