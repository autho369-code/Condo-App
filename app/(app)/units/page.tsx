  ...
  return (
    <div className="mx-auto max-w-5xl px-8 py-6 space-y-4">
      ...
      <form action="/units" method="get" className="flex flex-wrap items-center gap-2 rounded border border-gray-200 bg-white px-4 py-3">
        ...
        <select name="filter" defaultValue={filter ?? ''} className="h-9 rounded border border-gray-300 bg-white px-3 text-sm">
      ...
      {filtered.length > 0 ? (
        <Table>
          ...
          <tbody>
            {filtered.map((u: any) => (
              <TR key={u.unit_id}>
                <TD className="text-sm text-gray-700">{u.association_name}</TD>
                <TD className="font-medium"><Link href={`/units/${u.unit_id}`} className="text-blue-700 hover:underline">Unit {u.unit_number}</Link></TD>
