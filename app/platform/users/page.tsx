  ...
  return (
    <div className="space-y-7">
      <header>
        ...
        <p className="mt-1 text-sm text-gray-500">Global staff and portal identity oversight by client.</p>
      </header>
        ...
        <CardBody className="p-0">
          <Table className="border-0">
            ...
            <tbody>
              {(users ?? []).length === 0 ? (
                <TR>
                  <TD colSpan={5} className="py-10 text-center text-gray-500">No users are visible.</TD>
                </TR>
              ) : (
                (users ?? []).map((user: any) => (
                  <TR key={user.id} className="hover:bg-gray-50">
                    <TD>
                      <div className="font-medium text-gray-950">{user.full_name ?? user.display_name ?? user.email ?? 'Unnamed user'}</div>
                      <div className="mt-1 text-xs text-gray-500">{user.email ?? 'No email'}</div>
                    </TD>
                    <TD>
                      {user.portfolio_id ? (
                        <Link href={`/platform/portfolios/${user.portfolio_id}`} className="text-blue-700 hover:underline">
